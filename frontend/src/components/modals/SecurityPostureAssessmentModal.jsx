...
const SecurityPostureAssessmentModal = ({ isOpen, onClose, onAssessmentCreated = () => { }, selectedClient = null }) => {
    ...
    const getSelectedTypesText = () => {
        ...
        if (selectedTypes.size === 1) {
            ...
            const type = securityAssessmentTypes.find(t => t.id === typeId);
    ...
    const getEstimatedTotalTime = () => {
        const selectedTypeObjects = securityAssessmentTypes.filter(t => selectedTypes.has(t.id));
        ...
        const totalMinutes = selectedTypeObjects.reduce((sum, type) => {
            ...
            const minutes = parseInt(timeStr.split('-')[1] || timeStr.split('-')[0]) || 5;
            return sum + minutes;
        }, 0);
    ...
    const handleSubmit = async () => {
        try {
            ...
            console.error('Failed to create security assessment(s):', error);
            if (error.response?.status === 402) {
                setError('Assessment limit reached. Please upgrade your subscription or contact support.');
            } else if (error.response?.data?.error) {
                setError(error.response.data.error);
            } else {
                setError('Failed to create security assessment(s). Please try again.');
            }
        } finally {
            setIsCreating(false);
        }
            ...
            <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex items-center justify-between p-6 border-b border-gray-700">
                    <div className="flex items-center space-x-3">
                        ...
                        <h2 className="text-xl font-semibold text-white">Security Posture Assessment</h2>
                    </div>
                    <button
                        onClick={handleClose}
                    ...
                    <div className="flex items-center justify-between">
                        {[1, 2, 3].map((stepNumber) => (
                            <div key={stepNumber} className="flex items-center">
                                ...
                                {stepNumber < 3 && (
                                    <div className={`w-16 h-0.5 ml-4 ${step > stepNumber ? 'bg-red-500' : 'bg-gray-600'
                                        }`} />
                                )}
                            </div>
                        ))}
                    ...
                    {loading ? (
                        ...
                        <>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-gray-300 mb-3">
                                            Security Assessment Types <span className="text-red-400">*</span>
                                        </label>
                                        <div className="space-y-3">
                                            {securityAssessmentTypes.map((type) => (
                                                <div
                                                    key={type.id}
                                        ...
                                        <div className="mt-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                                            <div className="flex items-center">
                                                <Info className="text-red-400 mr-2" size={16} />
                                        ...
                                        <div className="p-4 bg-gray-700/30 border border-gray-600 rounded-lg">
                                            <label className="flex items-start">
                                                <input
                                                    ...
                                                    className="mt-1 mr-3 w-4 h-4 text-red-500 bg-gray-700 border-gray-600 rounded focus:ring-red-500 focus:ring-2"
                                                />
                                                <div>
                                                    <span className="text-sm font-medium text-gray-300">
                                                        Use client-specific security preferences
                                                    </span>
                                                    <p className="text-xs text-gray-400 mt-1">
                                                        Assessment will apply client-specific security policies, compliance requirements, and risk tolerance settings.
                                                    </p>
                                                </div>
                                            </label>
                                        </div>
                            ...
                            {step === 3 && (
                                <div className="space-y-6">
                                    <div>
                                        <h3 className="text-lg font-medium text-white mb-4">Review Security Assessment Details</h3>
                                        <div className="space-y-4">
                                            ...
                                            {currentClient && (
                                                <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-lg">
                                                    <div className="flex items-center mb-2">
                                                        <Info className="text-red-400 mr-2" size={16} />
                                                        <span className="text-sm font-medium text-red-400">Security Assessment for Client</span>
                                                    </div>
                                                    ...
                                                    {useClientPreferences && (
                                                        <p className="text-sm text-gray-300 mt-1">
                                                            <strong>Security Preferences:</strong> Client-specific security standards will be applied
                                                        </p>
                                                    )}
                                                </div>
                                            )}
                                            ...
                                            <div className="p-4 bg-gray-700/50 border border-gray-600 rounded-lg">
                                                <h4 className="font-medium text-white mb-2">Selected Security Assessments:</h4>
                                                <ul className="text-sm text-gray-300 space-y-1">
                                                    ...
                                                    {useClientPreferences && (
                                                        <li>• Client-specific security policies and compliance requirements</li>
                                                    )}
                                                </ul>
                                            </div>
                                        </div>
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        {step < 3 ? (
                            ...
                            <button
                                ...
                                disabled={isCreating}
                                className="px-6 py-2 bg-red-500 text-white rounded-md hover:bg-red-600 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
                            >
                                {isCreating ? (
                                    <>
...
export default SecurityPostureAssessmentModal;